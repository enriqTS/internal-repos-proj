...
├── docs/
│   └── arquitetura/
│       └── <diagrama>.svg              # Diagrama de arquitetura em SVG (obrigatório)
...
- **Formato obrigatório:** `.svg` (exportado do draw.io ou ferramenta equivalente)
- O CI detecta automaticamente o arquivo `architecture.svg` na raiz do diretório do template no S3 para exibição na plataforma
- Recomendação: mantenha o `.drawio` (editável) junto ao `.svg` exportado no mesmo diretório
- O diagrama será exibido na página de detalhes do template com suporte a lightbox
- **PNG não é aceito** — SVG garante qualidade em qualquer resolução e é mais leve
...
- [ ] Diagrama de arquitetura em `docs/arquitetura/` (SVG obrigatório)
