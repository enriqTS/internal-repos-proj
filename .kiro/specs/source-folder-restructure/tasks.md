...
- [x] 1. Restructure frontend components
  - [x] 1.1 Create `frontend/src/components/` directory and move component files
  ...
  - [x] 1.2 Verify frontend build after component move
...
- [x] 2. Restructure frontend pages
  - [x] 2.1 Create `frontend/src/pages/` directory and move page files
  ...
  - [x] 2.2 Verify frontend build after page move
    - Run `tsc --noEmit` in `frontend/` to confirm all imports resolve
    - Run `npm run build` in `frontend/` to confirm Vite bundling succeeds
    - _Requirements: 2.3, 6.2_

- [x] 3. Restructure frontend utilities
  - [x] 3.1 Create `frontend/src/utils/` directory and move utility files
  ...
  - [x] 3.2 Verify frontend build and tests after utility move
    - Run `tsc --noEmit` in `frontend/` to confirm all imports resolve
    - Run `npm run build` in `frontend/` to confirm Vite bundling succeeds
    - _Requirements: 6.2, 6.4_

- [x] 4. Checkpoint - Verify full frontend integrity
  - Ensure all tests pass, ask the user if questions arise.

- [x] 5. Restructure backend handlers
  - [x] 5.1 Create `lambda/src/handlers/` directory and move handler files
  ...
  - [x] 5.2 Update `lambda/package.json` esbuild entry points
  ...
  - [x] 5.3 Verify backend build after handler move
...
- [x] 6. Restructure backend utilities
  - [x] 6.1 Create `lambda/src/utils/` directory and move utility files
    - Create the `utils/` subfolder
  ...
  - [x] 6.2 Verify backend build after utility move
    - Run `tsc --noEmit` in `lambda/` to confirm all imports resolve
    - Run `npm run build` in `lambda/` to confirm esbuild bundling succeeds
