...
- [x] 1. Phase 1: Infrastructure Setup
  - [x] 1.1 Install Tailwind CSS 4 and the Vite plugin
    - Run `npm install tailwindcss @tailwindcss/vite` in the `frontend/` directory
    - Verify packages are added to `package.json`
    - _Requirements: 1.1, 1.2_

  - [x] 1.2 Create `frontend/src/styles.css` with Tailwind directives and theme tokens
  ...
  - [x] 1.3 Update `frontend/vite.config.ts` to register the Tailwind plugin
    - Import and register `@tailwindcss/vite` plugin
    - Keep existing alias and build configuration intact
    - _Requirements: 1.2, 1.3_

  - [x] 1.4 Import `styles.css` from the TypeScript entry point
...
- [x] 3. Phase 2: UI Helpers Module
  - [x] 3.1 Create `frontend/src/ui.ts` with all factory functions
...
- [x] 4. Checkpoint - Verify helpers module
  - Ensure all tests pass, ask the user if questions arise.

- [x] 5. Phase 3: Component Migration
  - [x] 5.1 Migrate `card-grid.ts`
  ...
  - [x] 5.3 Migrate `main.ts` (layout and header styling)
  ...
  - [x] 5.15 Migrate `templates-page.ts`
    - Replace custom class names with Tailwind utility classes
    - Use `container()`, `heading()`, `card()`, `badge()` helpers
    - _Requirements: 5.2, 5.3, 5.5, 8.1_

  - [x] 5.16 Migrate `theme-manager.ts`
...
- [x] 7. Phase 4: Legacy Style Cleanup
  - [x] 7.1 Remove the `<style>` block from `frontend/index.html`
  ...
  - [x] 7.2 Confirm `card-grid.ts` has no `injectStyles` remnants
  ...
  - [x] 7.3 Final verification — sole source of styling
...
- [x] 8. Final checkpoint - Full regression check
