"""FastAPI application entry point for ECS WebSocket chatbot service — streaming variant.
...
- POST /ws/message — handle sendMessage route (streams AI response chunks via @connections)

The API Gateway WebSocket API forwards lifecycle events as HTTP POST requests
to the ECS service via VPC Link -> NLB. The ECS service processes messages,
invokes AgentCore in streaming mode, and forwards chunks progressively back to
the client via the API Gateway Management API (@connections endpoint).
...
- MAX_CHUNK_SIZE: Maximum tokens per WebSocket frame (1-50, default: 1)
"""
...
app = FastAPI(
    title="Chatbot RAG — Bedrock AgentCore (ECS, WebSocket, Streaming)",
    version="0.1.0",
    lifespan=lifespan,
)
...
async def ws_message(request: Request) -> JSONResponse:
    """Handle sendMessage route event forwarded via VPC Link — streaming mode.

    Processes the user message, invokes AgentCore in streaming mode, and sends
    each chunk progressively to the client via the API Gateway Management API
    (@connections). After stream completes, sends {"type": "done"} and saves
    the assembled response to conversation history.

    If the client disconnects mid-stream (send_to_connection returns False),
    the stream is aborted and the partial response is discarded.
    ...
    Returns:
        200 on success (response delivered via @connections as stream chunks).
    """
    correlation_id = str(uuid.uuid4())
    logger.append_keys(correlation_id=correlation_id)
    ...
    logger.info(
        "Processing WebSocket message (streaming)",
        extra={
            "connectionId": connection_id,
            "userId": user_id,
    ...
    try:
        ...
        return JSONResponse(
            status_code=200,
            content={"statusCode": 200, "body": "Message processed (streaming)"},
        )
