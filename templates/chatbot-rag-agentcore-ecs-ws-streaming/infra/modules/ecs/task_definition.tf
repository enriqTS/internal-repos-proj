################################################################################
# ECS Task Definition — WebSocket Streaming Variant
################################################################################
...
resource "aws_ecs_task_definition" "chatbot" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "POWERTOOLS_SERVICE_NAME", value = "chatbot-ecs-ws-streaming" },
      ...
      { name = "MAX_CHUNK_SIZE", value = tostring(var.max_chunk_size) },
    ]
