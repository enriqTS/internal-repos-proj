...
AgentCore Runtime handles tool-use orchestration internally and **manages conversation context natively via `sessionId`** — the application consumes the streaming completion event-by-event and forwards text chunks to the client as they arrive. Conversation exchanges are persisted to DynamoDB after stream completion for compliance and audit purposes.
