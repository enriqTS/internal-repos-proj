...
variable "model_id" {
  description = "Bedrock model identifier (e.g., 'us.anthropic.claude-sonnet-4-20250514')"
  type        = string
...
variable "max_tool_iterations" {
  description = "Maximum tool-use loop iterations before aborting"
  type        = number
  default     = 10
}
...
variable "max_chunk_size" {
  description = "Maximum tokens per WebSocket frame (1-50). Allows batching tokens to reduce frame overhead."
  type        = number
  default     = 1
