"""FastAPI application entry point for ECS WebSocket chatbot service (streaming).
...
service through a VPC Link + NLB integration. The streaming variant uses the
Mantle API with stream=True and progressively delivers tokens to the client.
...
app = FastAPI(
    title="Chatbot RAG — Bedrock Mantle (ECS, WebSocket, Streaming)",
    version="0.1.0",
    lifespan=lifespan,
)
