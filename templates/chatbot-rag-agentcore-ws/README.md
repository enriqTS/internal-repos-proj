# Chatbot RAG Template — Bedrock AgentCore Runtime (WebSocket)
...
6. Orchestrator sends the final response to the client via WebSocket
...
│   ├── orchestrator/
│   │   └── handler.py                  # SQS-triggered, pushes response via WebSocket
│   ├── ai_caller/
...
2. Register the tool as an action group in the AgentCore module (`infra/modules/agentcore/`)
