################################################################################
# Bedrock AgentCore — Agent, Alias, and Action Group (ECS variant)
################################################################################
...
# Action Group — uses RETURN_CONTROL for ECS in-process tool execution
resource "aws_bedrockagent_agent_action_group" "tools" {
  agent_id          = aws_bedrockagent_agent.chatbot.agent_id
  agent_version     = "DRAFT"
  action_group_name = "${local.name_prefix}-tools"
  description       = "Action group for RAG tool execution (return control to ECS app)"
