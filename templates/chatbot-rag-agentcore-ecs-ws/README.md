# Chatbot RAG Template — Bedrock AgentCore (ECS, WebSocket)

## Overview

A chatbot RAG template using Bedrock AgentCore Runtime for AI inference, ECS Fargate for compute, and WebSocket for real-time bidirectional communication. Non-streaming: the AI generates the complete response before sending it to the client as a single WebSocket message.
...
Client → API Gateway WebSocket → VPC Link → NLB → ECS Fargate (FastAPI)
                                                       ├── Orchestrator (single-call pattern)
                                                       ├── AI Caller (AgentCore Runtime)
...
└── src/
    └── app/
        ...
        ├── orchestrator.py      # Conversation flow (single-call to AgentCore)
        ├── ai_caller.py         # AgentCore Runtime invocation
